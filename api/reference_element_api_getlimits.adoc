...
summary: 'Vous pouvez utiliser la méthode GetLimits pour obtenir les valeurs limites définies par l"API.  Ces valeurs peuvent changer d"une version à l"autre d"Element, mais elles ne changent pas sans une mise à jour du système.  Connaître les valeurs limites définies par l"API peut s"avérer utile lors de l"écriture de scripts API pour les outils destinés aux utilisateurs.' 
---
= ObtenirLimites
:allow-uri-read: 
:icons: font
...
NOTE: Le `GetLimits` Cette méthode renvoie les limites de la version logicielle actuelle, quelle que soit la version du point de terminaison de l'API utilisée pour transmettre la méthode.
...
Cette méthode renvoie un objet JSON contenant des paires nom-valeur définissant les limites de l'API.
