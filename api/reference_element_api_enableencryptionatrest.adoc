...
[NOTE]
====
* Pour consulter l'état actuel du chiffrement au repos et/ou du chiffrement logiciel au repos sur le cluster, utilisezlink:../api/reference_element_api_getclusterinfo["méthode d'obtention d'informations sur le cluster"^] . Vous pouvez utiliser le `GetSoftwareEncryptionAtRestInfo` link:../api/reference_element_api_getsoftwareencryptionatrestinfo["méthode pour obtenir des informations sur la manière dont le cluster chiffre les données au repos"^].
