---
permalink: api/reference_element_api_getsslcertificate.html 
sidebar: sidebar 
keywords: SSL,certificate,cert,get,getting,active,GetSSLCertificate 
summary: Vous pouvez utiliser la méthode GetSSLCertificate pour récupérer le certificat SSL actuellement actif sur les nœuds de stockage du cluster. 
---
= Obtenir le certificat SSL
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Vous pouvez utiliser le `GetSSLCertificate` méthode permettant de récupérer le certificat SSL actuellement actif sur les nœuds de stockage du cluster.



== Paramètres

Cette méthode ne requiert aucun paramètre d'entrée.



== Valeurs de retour

Cette méthode renvoie les valeurs suivantes :

|===
| Nom | Description | Type 


 a| 
certificat
 a| 
Le texte intégral du certificat encodé au format PEM.
 a| 
chaîne



 a| 
détails
 a| 
Informations décodées du certificat.
 a| 
objet JSON

|===


== Exemple de demande

Les requêtes pour cette méthode sont similaires à l'exemple suivant :

[listing]
----
{
    "method" : "GetSSLCertificate",
    "params" : {},
    "id" : 1
}
----


== Exemple de réponse

Cette méthode renvoie une réponse similaire à l'exemple suivant :

[listing]
----
{
    "id": 1,
    "result": {
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n",
        "details": {
            "issuer": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org",
            "modulus": "",
            "notAfter": "2027-03-06T22:50:26Z",
            "notBefore": "2017-03-08T22:50:26Z",
            "serial": "CC1B221598E37FF3",
            "sha1Fingerprint": "1D:70:7A:6F:18:8A:CD:29:50:C7:95:B1:DD:5E:63:21:F4:FA:6E:21",
            "subject": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org"
        }
    }
}
----


== Nouveautés depuis la version

10,0
