...
[role="lead"]
Vous pouvez utiliser le `ListAuthSessionsByClusterAdmin` méthode permettant de lister toutes les sessions d'authentification associées à la session spécifiée `ClusterAdminID` .  Si le spécifié `ClusterAdminID` Lorsqu'un utilisateur est associé à un groupe, toutes les sessions d'authentification de tous les membres de ce groupe seront listées.
