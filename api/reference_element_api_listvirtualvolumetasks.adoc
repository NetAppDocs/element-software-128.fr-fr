...
summary: Vous pouvez utiliser la méthode ListVirtualVolumeTasks pour obtenir la liste des tâches de volume virtuel du système. 
---
= Lister les tâches de volume virtuel
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez utiliser le `ListVirtualVolumeTasks` Méthode permettant d'obtenir la liste des tâches de volume virtuel dans le système.
...
ID de tâche de volume virtuel
 a| 
Liste des identifiants de tâches de volume virtuel pour lesquels récupérer des informations.  Si vous omettez ce paramètre, la méthode renvoie des informations sur toutes les tâches de volume virtuel.
 a| 
tableau UUID
 a| 
Aucune
...
tâches
 a| 
Liste des objets décrivant les tâches de volume virtuel dans le cluster.
 a| 
xref:reference_element_api_task_virtual_volumes.adoc[tâche]tableau
